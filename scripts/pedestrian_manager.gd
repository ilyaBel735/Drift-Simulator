...
@export var parking_spawn_chance := 0.25
...
func _try_spawn_one() -> void:
	...
	for attempt in 40:
		...
		if randf() < parking_spawn_chance and city.has_method("get_random_parking_spot"):
			pos = city.get_random_parking_spot()
			if pos.x > 900000.0:
				continue
			block = Vector2i(int(floor(pos.x / spacing)), int(floor(pos.z / spacing)))
		else:
			block = _random_block_near_player(spacing)
			if not city.is_pedestrian_chunk(block):
				continue
			corner = randi_range(0, 3)
			pos = _corner_point(block, corner, spacing, city.road_width)
