...
enum State { IDLE, WALK, WAIT, CROSS, TO_DOOR, INSIDE, EXIT_DOOR }
...
@export var door_enter_chance := 0.30
@export var inside_time_min := 6.0
@export var inside_time_max := 20.0
...
var door_target := Vector3.ZERO
var return_point := Vector3.ZERO
var inside_time := 0.0

@onready var col_shape := get_node_or_null("CollisionShape3D") as CollisionShape3D
...
func _physics_process(delta: float) -> void:
	match state:
		...
		State.WALK, State.CROSS, State.TO_DOOR, State.EXIT_DOOR:
...
func _on_target_reached() -> void:
	...
	elif state == State.TO_DOOR:
		_enter_building()
	elif state == State.EXIT_DOOR:
		state = State.IDLE
		idle_time = randf_range(0.5, 2.0)

func _choose_action_at_corner() -> void:
	if randf() < door_enter_chance and _try_enter_building():
		return
...
func _try_enter_building() -> bool:
	if not is_instance_valid(city):
		return false
	if not city.has_method("get_nearby_door"):
		return false
	var door: Vector3 = city.get_nearby_door(global_position, 35.0)
	if door.distance_to(global_position) > 35.0:
		return false
...
func _enter_building() -> void:
	...
	velocity = Vector3.ZERO
	state = State.INSIDE
	inside_time = randf_range(inside_time_min, inside_time_max)

func _exit_building() -> void:
	visible = true
	if col_shape:
		col_shape.disabled = false
	target_point = return_point
	state = State.EXIT_DOOR
